...
class ResultsScreen extends FlxSubState
{
	...
	{	
        ...
        if (PlayState.storyWeek == 7)
            {
                music = new FlxSound().loadEmbedded(Paths.music('nebulaintro'), false, true);
            }
        else
            {
        ...
        if (PlayState.storyWeek == 7 && PlayState.isStoryMode)
            {
                comboText = new FlxText(20,-75,0,'Judgements:\nSicks - ${PlayState.sicks}\nGoods - ${PlayState.goods}\nBads - ${PlayState.bads}\n\nCombo Breaks: ${(PlayState.isStoryMode ? PlayState.campaignMisses : PlayState.misses)}\nHighest Combo: ${PlayState.highestCombo + 1}\n\nScore: ${PlayState.instance.songScore}\nAccuracy: ${HelperFunctions.truncateFloat(PlayState.instance.accuracy,2)}%  You can now use a purple mic from the options\n\n${Ratings.GenerateLetterRank(PlayState.instance.accuracy)}\n\nF1 - View replay\nF2 - Replay song
                ');
            }
        else
            {
        ...
        FlxG.sound.play(Paths.sound('results'));
	...
	override function update(elapsed:Float)
	{
        ...
        if (!music.playing)
            {
                music = new FlxSound().loadEmbedded(Paths.music('nebula'), true, true);
                music.play(false, FlxG.random.int(0, Std.int(music.length / 2)));
            }
