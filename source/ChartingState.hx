...
class ChartingState extends MusicBeatState
{
	...
	public static var assSettings:Array<String>;
	var curWhat:String;
	var timeAuto:Float;
	...
	var curRenderedSelections:FlxTypedGroup<FlxSprite>;
	...
	**/
	...
	public var sysTextm:FlxText;

	override function create()
	{
		assSettings = getAssSettings();
		trace("Hello");
		timeAuto = Std.parseFloat(assSettings[4]);
		...
		sysTextm = new FlxText(60,40,0,curWhat, 14);
		sysTextm.scrollFactor.set();
		...
		curRenderedSelections = new FlxTypedGroup<FlxSprite>();
	...
	var check_chor:FlxUICheckBox;
	var check_changeBPM:FlxUICheckBox;
	var stepperSectionBPM:FlxUINumericStepper;
	var check_altAnim:FlxUICheckBox;

	function getAssSettings():Array<String>
	{
		var fullText:String = Assets.getText(Paths.txt('assSettings'));
	...
	function addSectionUI():Void
	{
		...
		var stepperCopyB:FlxUINumericStepper = new FlxUINumericStepper(110, 192, 1, 1, -999, 999, 0);
		var stepperCopyBLabel = new FlxText(174,192,'chorus (only 1 section for now :((((((');
		var DeleteIfUrGay = new FlxText(10,250,'New charting system by 4Axion');

		var copyButtonB:FlxButton = new FlxButton(10, 190, "Paste chorus", function()
		{
			copyChorus(Std.int(stepperCopyB.value));
		});
		...
		check_chor = new FlxUICheckBox(174, 30, null, null, "Chorus?", 100);
		check_chor.name = 'check_chor';
		check_chor.checked = false;
		...
		//tab_group_section.add(check_chor);
		tab_group_section.add(check_altAnim);
		tab_group_section.add(check_changeBPM);
		tab_group_section.add(copyButton);
		tab_group_section.add(clearSectionButton);
		tab_group_section.add(swapSection);
		tab_group_section.add(stepperCopyB);
		tab_group_section.add(stepperCopyBLabel);
		tab_group_section.add(copyButtonB);
		tab_group_section.add(DeleteIfUrGay);
	...
	override function update(elapsed:Float)
	{
		...
		sysTextm.text = curWhat;

		curStep = recalculateSteps();
		if(!saving)
			{
				if(assSettings[7] == "true")
				{
				curWhat = Std.string(timeAuto) + "s";
				}
				else
				{
				curWhat = "Idle..."; //this is working dumbass
				}
			}
		else
			{
		...
		if (timeAuto < 1)
			...
			//curSelectedNote[0].selected = true;
			// to save: saveLevel();
	...
	function copyChorus(?sectionNum:Int = 1)
	{
	...
	function updateGrid():Void
	{
		...
		while (curRenderedSelections.members.length > 0)
		{
			curRenderedSelections.remove(curRenderedSelections.members[0], true);
		}
		...
		for (i in sectionInfo)
		{
			...
			//curSelectedNote[0].color = FlxColor.ORANGE;

			curRenderedNotes.add(note);

			if(i.selected)
			{
				var colMan:String = ChartingState.assSettings[4];
				i.color = Std.parseInt(colMan);
			}
			else
			{
	...
	private function newSection(lengthInSteps:Int = 16,mustHitSection:Bool = false,chor:Bool = false,altAnim:Bool = true):SwagSection
		{
			var sec:SwagSection = {
				lengthInSteps: lengthInSteps,
				bpm: _song.bpm,
	...
	private function addNote(?n:Note):Void
	{
		...
		var noteData = Math.floor(FlxG.mouse.x / GRID_SIZE); //markinggg!!!!!! very importtasnnzzzz!!11!!111
		var noteSus = 0;
